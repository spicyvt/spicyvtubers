(() => {
  ...
  const headerDropdownClosers = [];
  function closeOtherHeaderDropdowns(exceptCloser) {
    headerDropdownClosers.forEach((closer) => {
      if (closer !== exceptCloser) closer();
    });
  }
  ...
  function initPageSearch() {
    ...
    function openPanel() {
      closeOtherHeaderDropdowns(closePanel);
      panel.hidden = false;
    ...
    headerDropdownClosers.push(closePanel);
  ...
  (() => {
    ...
    function openMenu() {
      closeOtherHeaderDropdowns(closeMenu);
      menu.hidden = false;
      toggleBtn.setAttribute("aria-expanded", "true");
    }

    headerDropdownClosers.push(closeMenu);
  ...
  function initCreatorPage() {
    ...
    initFanslyGraphs();
    initStreamHistory();
    applyCreatorPageLiveStatus();
  }

  
  
  
  async function applyCreatorPageLiveStatus() {
    const slug = document.body.dataset.slug;
    if (!slug) return;
    const liveLoginsByPlatform = await loadAllLiveLogins();
    LIVE_STATUS_PLATFORMS.forEach((platform) => {
      if (!liveLoginsByPlatform[platform.key].has(slug)) return;
      const pill = document.querySelector(`.spice-handles .spice-pill[data-icon="${platform.key}"]`);
      if (pill) pill.classList.add(`is-live-${platform.key}`);
    });
  }
