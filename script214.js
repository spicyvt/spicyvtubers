(() => {
  ...
  const LIVE_JSON_URL = "https://cdn.spicyvtubers.com/fansly/live.json";
  const LIVE_STALE_MS = 24 * 60 * 60 * 1000;

  // Fetches fansly/live.json and returns the set of lowercased channel logins currently
  ...
  function buildRow(creator) {
    ...
    const spicePills = SPICE_PLATFORMS.filter((platform) => creator[platform.key])
      .map((platform) => {
        ...
        const pillClass = platform.key === "fansly" && creator.isLive ? "spice-pill is-live-fansly" : "spice-pill";
  ...
  function initIndex() {
    ...
    // Clears each creator's isLive flag and reapplies it from a fresh live-logins Set,
    // invalidating the cached row of anyone whose live status actually changed so their
    // pill re-renders (rebuilt on the next getOrBuildRow() call, inside update()).
