(() => {
  ...
  function initIndex() {
    ...
    if (!tableSection || !newestSection || !thead || !tbody || !searchInput || !platformFilterEl || !sortToggleBtn) return;
    ...
    function update() {
      ...
      rows.forEach((row) => {
        ...
        const matches = rowMatches(row, query);
        row.hidden = !matches;
      ...
      const isSolo = visibleRows.length === 1;
      thead.hidden = isSolo;
      const profileRowEl = getProfileRow();
      profileRowEl.hidden = !isSolo;
      if (isSolo) {
        const soloRow = visibleRows[0];
      ...
      updateResultCount(visibleRows.length, totalCount);
    }
