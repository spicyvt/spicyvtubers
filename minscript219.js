(() => {
  ...
  function initFanslyGraphs() {
    ...
    const resetBtn = document.querySelector(".fansly-graph-reset");
    ...
    if (resetBtn) resetBtn.addEventListener("click", () => resetChartZoom(chart));
  ...
  function renderStreamOverviewGraph(container, streams) {
    ...
    const hasAvg = (s) => typeof s.avgViewers === "number" && s.avgViewers > 0;
    const chronological = streams
      .filter((s) => typeof s.startedAt === "number" && (hasPeak(s) || hasAvg(s)))
      .slice()
    ...
    resetBtn.className = "chart-reset-btn stream-overview-reset";
    resetBtn.textContent = "Reset zoom";
    container.appendChild(resetBtn);

    const peakPoints = chronological.filter(hasPeak).map((s) => ({ x: s.startedAt, y: s.maxViewers }));
    const avgPoints = chronological.filter(hasAvg).map((s) => ({ x: s.startedAt, y: s.avgViewers }));

    const chart = new Chart(canvas, {
      type: "line",
      data: {
        datasets: [
          { label: "Peak", data: peakPoints, borderColor: CHART_LINE_COLOR, backgroundColor: CHART_LINE_COLOR, pointRadius: 3, pointHoverRadius: 5 },
          { label: "Avg", data: avgPoints, borderColor: "#7cc4fb", backgroundColor: "#7cc4fb", pointRadius: 3, pointHoverRadius: 5 },
        ],
      },
      options: {
        responsive: true,
        ...
        plugins: {
          ...
          tooltip: {
            ...
            padding: 8,
            callbacks: {
              title: (items) => formatEpochDate(items[0].parsed.x),
              label: (item) => `${item.dataset.label}: ${item.parsed.y} viewers`,
            },
          },
          zoom: CHART_ZOOM_PAN_PLUGIN,
        },
    ...
    resetBtn.addEventListener("click", () => resetChartZoom(chart));
  }
  ...
  async function loadStreamHistory(container) {
    ...
    try {
      ...
      renderStreamOverviewGraph(container, ordered);
  ...
  async function initLeaderboardPage() {
    ...
    const resetZoomBtn = document.getElementById("leaderboard-reset-zoom-btn");
    ...
    if (resetZoomBtn) resetZoomBtn.addEventListener("click", () => resetChartZoom(leaderboardChart));
